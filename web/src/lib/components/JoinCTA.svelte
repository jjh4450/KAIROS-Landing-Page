...
<section id="join" class="relative mx-auto w-full max-w-7xl px-6 py-24 lg:py-32">
	<div class="tilt-3d reveal">
	<div use:cardTilt={{ max: 3, scale: 1.005 }} class="tilt-3d-card glass relative overflow-hidden p-8 md:p-12">
		<!-- corner ticks -->
		<span class="border-kairos-cyan absolute top-0 left-0 h-3 w-3 border-t border-l"></span>
		<span class="border-kairos-cyan absolute top-0 right-0 h-3 w-3 border-t border-r"></span>
		<span class="border-kairos-cyan absolute bottom-0 left-0 h-3 w-3 border-b border-l"></span>
		<span class="border-kairos-cyan absolute right-0 bottom-0 h-3 w-3 border-r border-b"></span>
		...
		<div class="relative flex flex-col gap-8 md:flex-row md:items-end md:justify-between">
			<div class="max-w-2xl">
				...
				<p class="text-muted-foreground text-base md:text-lg">
					...
					{:else}
						현재는 신규 모집을 받고 있지 않습니다. SNS를 팔로우하시면 다음 모집 공고를 가장 먼저 받아보실 수 있습니다.
					{/if}
				</p>
